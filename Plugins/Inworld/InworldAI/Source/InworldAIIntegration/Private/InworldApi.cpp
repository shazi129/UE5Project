    ...
    : UWorldSubsystem()
{
    ...
    Client.InitClient(
        ...
        [this](std::shared_ptr<Inworld::Packet> Packet)
        {
            ...
            auto SourceName = FString(Packet->_Routing._Source._Name.c_str());
            auto* SourceComponentPtr = CharacterComponentByAgentId.Find(SourceName);
            if (SourceComponentPtr)
            {
                (*SourceComponentPtr)->HandlePacket(Packet);
            }

			auto TargetName = FString(Packet->_Routing._Target._Name.c_str());
            auto* TargetComponentPtr = CharacterComponentByAgentId.Find(TargetName);
...
void UInworldApiSubsystem::StartSession(const FString& SceneName, const FString& PlayerName, const FString& ApiKey, const FString& ApiSecret, const FString& AuthUrlOverride, const FString& TargetUrlOverride)
{
    ...
    Client.StartClient(Options,
        [this](const auto& AgentInfos)
        {
            ...
            {
                FString BrainName = UTF8_TO_TCHAR(Info.BrainName.c_str());
                auto* ComponentPtr = CharacterComponentRegistry.FindByPredicate([&BrainName](const auto C) { return C->GetBrainName() == BrainName; });
                if (ComponentPtr)
                {
                    auto& Component = *ComponentPtr;
                    auto AgentId = FString(Info.AgentId.c_str());
                    CharacterComponentByAgentId.Add(AgentId, Component);
                    Component->SetAgentId(AgentId);
                    Component->SetGivenName(Info.GivenName.c_str());
                }
                AgentInfoByBrain.Add(Info.BrainName.c_str(), Info);
            }
...
{
    for (auto* Component : CharacterComponentRegistry)
    {
        Component->SetAgentId("");
        Component->SetGivenName("");
    }
...
void UInworldApiSubsystem::RegisterCharacterComponent(Inworld::ICharacterComponent* Component)
{
    ...
    else
    {
        ...
        {
            CharacterComponentRegistry.Add(Component);
            auto AgentId = FString(AgentInfoPtr->AgentId.c_str());
            CharacterComponentByAgentId.Add(AgentId, Component);
            Component->SetAgentId(AgentId);
            Component->SetGivenName(AgentInfoPtr->GivenName.c_str());
        }
    }
}
...
void UInworldApiSubsystem::UnregisterCharacterComponent(Inworld::ICharacterComponent* Component)
{
    ...
    const FString AgentId = Component->GetAgentId();
    if (CharacterComponentByAgentId.Contains(AgentId) && CharacterComponentByAgentId[AgentId] == Component)
    {
        CharacterComponentByAgentId.Remove(AgentId);
    }
}
...
void UInworldApiSubsystem::SendCustomEvent(FString AgentId, const FString& Name)
{
	if (!ensureMsgf(!AgentId.IsEmpty(), TEXT("AgentId must be valid!")))
	{
		return;
