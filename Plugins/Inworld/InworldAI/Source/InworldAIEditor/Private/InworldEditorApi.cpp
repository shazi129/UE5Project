...
void UInworldEditorApiSubsystem::SetupAssetAsInworldPlayer(const FAssetData& AssetData)
{
	...
	const TSubclassOf<UAudioCaptureComponent> PlayerAudioCaptureComponent = InworldAIEditorSettings->PlayerAudioCaptureComponent;
	if (PlayerAudioCaptureComponent)
	{
		auto* AudioCapture = Cast<UAudioCaptureComponent>(AddNodeToBlueprint(Blueprint, PlayerAudioCaptureComponent, PlayerAudioCaptureComponent->GetName()));
		if (AudioCapture)
		{
			AudioCapture->SoundSubmix = InworldAIEditorSettings->PlayerMuteSoundSubmix;
		}
	}
