...
void Inworld::ClientBase::StartClient(const ClientOptions& Options, std::function<void(const std::vector<AgentInfo>&)> LoadSceneCallback)
{
	...
	_AsyncGenerateTokenTask->Start(
		...
		std::make_unique<RunnableGenerateSessionToken>(
			...
			Options.ApiKey,
			Options.ApiSecret,
			[this, Options](const grpc::Status& Status, const InworldV1alpha::SessionAccessToken& Token)
			{
				AddTaskToMainThread([this, Options, Status, Token]() {
					if (!Status.ok())
					{
					...
					LoadScene(Token.token(), Token.session_id(), Options);
				});
			}
		)
	);
...
void Inworld::ClientBase::StartClient(const std::string& Token, const std::string& SessionId, const ClientOptions& Options, std::function<void(const std::vector<AgentInfo>&)> LoadSceneCallback)
{
	if (_ConnectionState != ConnectionState::Idle && _ConnectionState != ConnectionState::Failed)
	{
		return;
	...
	LoadScene(Token, SessionId, Options);
}
...
void Inworld::ClientBase::LoadScene(const std::string& Token, const std::string& SessionId, const ClientOptions& Options)
{
	Inworld::LogSetSessionId(SessionId);
