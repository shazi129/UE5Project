...
void UUIItem::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if (PropertyChangedEvent.Property != nullptr)
	{
		...
		if (PropertyName == GET_MEMBER_NAME_CHECKED(UUIItem, bIsUIActive) ||
			PropertyName == GET_MEMBER_NAME_CHECKED(UUIItem, bIsUIHidden))
		{
			bIsUIActive = !bIsUIActive;//make it work
			SetIsUIActive(!bIsUIActive);
		}
		else if (PropertyName == GET_MEMBER_NAME_CHECKED(UUIItem, bNotHitChildren))
		{
			bNotHitChildren = !bNotHitChildren;
			SetIsNotHitChildren(!bNotHitChildren);
		}
...
void UUIItem::UIHierarchyChanged(ULGUICanvas* ParentRenderCanvas, UUICanvasGroup* ParentCanvasGroup)
{
	...
	if (ParentUIItem.IsValid())
	{
		...
		this->bAllUpParentBeRaycastTarget = ParentUIItem->bAllUpParentBeRaycastTarget && !ParentUIItem->IsNotHitChildren();
		this->CheckUIActiveState();
	}
	else
	{
		this->bAllUpParentUIActive = true;
		this->bAllUpParentBeRaycastTarget = true;
		this->CheckUIActiveState();
	}
...
void UUIItem::CheckChildrenRaycastRecursive(bool inUpParentBeRaycastTarget)
{
	for (auto uiChild : UIChildren)
	{
		if (IsValid(uiChild))
		{
			uiChild->bAllUpParentBeRaycastTarget = inUpParentBeRaycastTarget;
			uiChild->CheckChildrenRaycastRecursive(inUpParentBeRaycastTarget);
		}
	}
}

void UUIItem::SetIsNotHitChildren(bool notHitChildren)
{
	if (bNotHitChildren != notHitChildren)
	{
		bNotHitChildren = notHitChildren;
		CheckChildrenRaycastRecursive(!bNotHitChildren);
	}
}
