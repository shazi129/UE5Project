...
void ULGUICanvas::BeginPlay()
{
	...
	if (CheckUIItem())
	{
		bPrevUIItemIsActive = UIItem->GetIsUIActiveInHierarchy() && !UIItem->GetIsUIHidden();
	}
	else
	{
...
void ULGUICanvas::UpdateRootCanvas()
{
	if (ensure(this == RootCanvas))
	{
		...
		if (CheckUIItem())
		{
			 ...
			 */
			const bool bNowUIItemIsActive = UIItem->GetIsUIActiveInHierarchy() && !UIItem->GetIsUIHidden();
...
void ULGUICanvas::UpdateGeometry_Implement()
{
	...
	for (int i = 0; i < UIRenderableList.Num(); i++)
	{
		...
		if (Item->GetIsUIHidden())continue;
...
FName ULGUICanvas::LGUI_TextureClipRotationAngle_MaterialParameterName = FName(TEXT("TextureClipRotationAngle"));
FName ULGUICanvas::LGUI_TextureClipType_MaterialParameterName = FName(TEXT("TextureClipType"));
	...
	auto FoundIndex = ParameterInfos.IndexOfByPredicate([InIncludeMainTexture](const FMaterialParameterInfo& Item)
		{
			return
				...
				|| Item.Name == LGUI_TextureClipRotationAngle_MaterialParameterName || Item.Name == LGUI_TextureClipType_MaterialParameterName
				;
		});
	...
	for (int i = 0; i < UIDrawcallList.Num(); i++)
	{
		...
		case ELGUICanvasClipType::Texture:
		{
				...
				|| this->bTextureClipParameterChanged)
			{
				...
				auto TempClipRotationAngle = this->GetTextureClipRotationAngle() / 360;
				auto TempClipTextureType = 1 - (int)this->GetTextureClipType();
				...
				case EUIDrawcallType::DirectMesh:
				{
					...
					if (RenderMaterial.IsValid())
					{
						...
						((UMaterialInstanceDynamic*)RenderMaterial.Get())->SetScalarParameterValue(LGUI_TextureClipRotationAngle_MaterialParameterName, TempClipRotationAngle);
						((UMaterialInstanceDynamic*)RenderMaterial.Get())->SetScalarParameterValue(LGUI_TextureClipType_MaterialParameterName, TempClipTextureType);
					}
				}
				break;
				case EUIDrawcallType::PostProcess:
...
FLinearColor ULGUICanvas::GetTextureClipOffsetAndSize()
{
	...
	auto Offset = FVector2D(UIItem->GetWidth() * (maskPivot.X - 0.5f * (1 + maskScale.X)), UIItem->GetHeight() * (maskPivot.Y - 0.5f * (1 + maskScale.Y)));
	return FLinearColor(Offset.X, Offset.Y, UIItem->GetWidth() * maskScale.X, UIItem->GetHeight() * maskScale.Y);
}
...
void ULGUICanvas::SetMaskPivot(FVector2D& vec)
{
	if (maskPivot != vec)
	{
		bTextureClipParameterChanged = true;
...
void ULGUICanvas::SetMaskScale(FVector2D& vec)
{
	if (maskScale != vec)
	{
		bTextureClipParameterChanged = true;
...
void ULGUICanvas::SetMaskRotation(float rotation)
{
	if (maskRotation != rotation)
	{
		bTextureClipParameterChanged = true;
...
void ULGUICanvas::SetMaskType(ELGUIMaskType type)
{
	if (maskType != type)
	{
		bTextureClipParameterChanged = true;
