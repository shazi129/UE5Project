...
void UUISpriteBase::ApplyAtlasTextureScaleUp()
{
	...
	if (IsValid(ueSprite))
	{
		geometry->texture = ueSprite->GetBakedTexture();
	}
	else
	{
...
void UUISpriteBase::SetUESprite(UPaperSprite* newSprite, bool setSize)
{
	if (ueSprite != newSprite)
	{
		//if ((!IsValid(ueSprite) || !IsValid(newSprite))
		//	|| (ueSprite->GetBakedTexture() != newSprite->GetBakedTexture()))
		//{
		//}
		ueSprite = newSprite;
		MarkTextureDirty();
...
UTexture* UUISpriteBase::GetTextureToCreateGeometry()
{
	...
	if (IsValid(ueSprite) && IsValid(ueSprite->GetBakedTexture()))
	{
		return ueSprite->GetBakedTexture();
	}
