...
void FLGUICanvasCustomization::CustomizeDetails(IDetailLayoutBuilder& DetailBuilder)
{
	...
	if (TargetScriptArray[0]->IsRootCanvas())
	{
		...
		if (clipType == ELGUICanvasClipType::None)
		{
			...
			needToHidePropertyNames.Add(GET_MEMBER_NAME_CHECKED(ULGUICanvas, maskPivot));
			needToHidePropertyNames.Add(GET_MEMBER_NAME_CHECKED(ULGUICanvas, maskScale));
			needToHidePropertyNames.Add(GET_MEMBER_NAME_CHECKED(ULGUICanvas, maskRotation));
			needToHidePropertyNames.Add(GET_MEMBER_NAME_CHECKED(ULGUICanvas, maskType));
		}
