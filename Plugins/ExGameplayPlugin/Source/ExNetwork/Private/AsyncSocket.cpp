...
FAsyncSocket::FAsyncSocket()
{
	...
	UE_LOG(LogClientNet, Log, TEXT("FAsyncSocket(). this:%p"), this);
}
...
void FAsyncSocket::InitAsyncSocket()
{
	...
	if (!_MainAccessOnly.bFirstReset)
	{
		auto destoryTask = [this]() {
			UE_LOG(LogClientNet, Log, TEXT("SocketThreadTask: destoryTask exec.Host:%s,Port:%d. this:%p"),
	...
	auto initTask = [this]() {
		UE_LOG(LogClientNet, Log, TEXT("SocketThreadTask: initSocket exec. this:%p"), this);
...
{
	auto connectTask = [=]() {
		UE_LOG(LogClientNet, Log, TEXT("SocketThreadTask: connectTask exec,Host:%s,Port:%d. This:%p"), *Host, Port, this);
...
void FAsyncSocket::ConnectByIp(const FInternetAddr& Addr, int Timeout)
{
	...
	{
		UE_LOG(LogClientNet, Warning, TEXT("ConnectIp state:%s"), StateToString(_AccessShared.State));
		return;
	}
	...
	UE_LOG(LogClientNet, Warning, TEXT("Connect to :%s"), *Addr.ToString(true));
...
void FAsyncSocket::Send(const char* Data, size_t Size)
{
	...
	{
		UE_LOG(LogClientNet, Error, TEXT("Send pkt failed socket state is %s"), StateToString(_AccessShared.State));
		return;
	}
	...
	if (Size > 8192)
	{
		UE_LOG(LogClientNet, Error, TEXT("pkt is more than 8K, drop it. size: %d"), Size);
		return;
	}

	_SocketAccessOnly.SendBuff.append(Data, Size);
	UE_LOG(LogClientNet, Log, TEXT("Send pkt, current size: %d, total size:%d"), Size, _SocketAccessOnly.SendBuff.size());

	auto sendTask = [=]() {
		UE_LOG(LogClientNet, Log, TEXT("SocketThreadTask: sendTask exec"));
...
void FAsyncSocket::Update()
{
	...
	auto updateTask = [this]() {
		// UE_LOG(LogClientNet, Log, TEXT("SocketThreadTask: updateTask exec"));
		...
		if (_AccessShared.State == ESocketState::Connected)
		{
			...
			{
				UE_LOG(LogClientNet, Log, TEXT("nifm state change to unavailable"));
...
void FAsyncSocket::Close()
{
	...
	auto closeTask = [this]() {
		UE_LOG(LogClientNet, Log, TEXT("SocketThreadTask: closeTask exec"));
		_AccessShared.State = ESocketState::Disconnected;
#if PLATFORM_SWITCH
		FRWScopeLock Lock(SocketNifmLock, SLT_ReadOnly);
#endif
...
{
	_AccessShared.State = ESocketState::Disconnected;
	UE_LOG(LogClientNet, Error, TEXT("%s"), *ErrorMsg);

	auto connectCallbackTask = [this, RetCode]() {
		UE_LOG(LogClientNet, Log, TEXT("MainThreadTask: connectCallbackTask exec, error"));
...
void FAsyncSocket::HandleError(const FString& ErrorMsg)
{
	...
	auto errorCallbackTask = [=]() {
		UE_LOG(LogClientNet, Log, TEXT("MainThreadTask: errorCallbackTask exec"));
...
void FAsyncSocket::CheckResolutionAndConnect(const FResolveInfo* ResolveInfo)
{
	...
	if (ResolveInfo->IsComplete())
	{
		...
		if (ErrorCode == 0)
		{
			...
			Addr->SetIp(IP);
			UE_LOG(LogClientNet, Warning, TEXT("DNS is complete, peer ip:%s"), *Addr->ToString(true));
...
bool FAsyncSocket::CheckIsConnected()
{
	...
	if (!FPlatformMisc::IsNifmAvailable())
	{
		// UE_LOG(LogClientNet, Log, TEXT("nifm is not available"));
		return false;
	}
#endif
	...
	if (bValid)
	{
		...
		else
		{
			...
			auto connectCallbackTask = [this]() {
				UE_LOG(LogClientNet, Log, TEXT("MainThreadTask: connectCallbackTask exec, success"));

				if (_MainAccessOnly.ConnecteCallBack)
				{
					UE_LOG(LogClientNet, Log, TEXT("Connect Success"));
...
bool FAsyncSocket::HandleRecv()
{
	...
	do
	{
		...
		if (ReadSize > 0)
		{
			...
			UE_LOG(LogClientNet, Log, TEXT("Recv bytes:%d"), ReadSize);
		}
	...
	while (_SocketAccessOnly.RecvBuff.size() > 0)
	{
		...
		UE_LOG(LogClientNet, Log, TEXT("Recv pkt, size:%d"), RecvPacket.size());

		auto recvCallbackTask = [=]() {
			UE_LOG(LogClientNet, Log, TEXT("MainThreadTask: recvCallbackTask exec"));
...
bool FAsyncSocket::HandleSend()
{
	...
	{
		UE_LOG(LogClientNet, Log, TEXT("Message length is more than 16K %d"), iSendLength);
		iSendLength = 16 * 1024;
	}
	...
	if (IsValid)
	{
		...
		UE_LOG(LogClientNet, Log, TEXT("Send bytes:%d"), SendSize);
	}
	else
	{
		ESocketErrors ErrorCode = _SocketAccessOnly.SocketSubsystem->GetLastErrorCode();
		...
		{
			UE_LOG(LogClientNet, Log, TEXT("SE_EWOULDBLOCK, Send bytes:%d"), SendSize);
		}
		else
		{
			HandleError(ErrorMsg);
