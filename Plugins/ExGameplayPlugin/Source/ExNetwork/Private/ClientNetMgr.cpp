...
{
	UE_LOG(LogClientNet, Warning, TEXT("---UClientNetMgr Deconstruct"));
}
...
{
	UE_LOG(LogClientNet, Warning, TEXT("---UClientNetMgr::HandleAppResumed"));
}
...
void UClientNetMgr::Tick(float DeltaTime)
{
	...
	if (m_pSocket && m_pSocket.IsValid())
	{
		...
		if (CheckIfHeartTimeout())//|| bAppResumedReconnect
		{
			...
			UE_LOG(LogClientNet, Warning, TEXT("---UClientNetMgr Heart beat time out"));
...
void UClientNetMgr::Send(const uint8* Data, uint32 Len)
{
	...
	{
		UE_LOG(LogClientNet, Error, TEXT("m_pSocket not valid"));
	}
}
